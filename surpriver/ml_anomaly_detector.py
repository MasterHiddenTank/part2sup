...
try:
    ...
    os.environ["CUDA_VISIBLE_DEVICES"] = "-1"  # Disable GPU to avoid CUDA/driver issues
...
class SimpleTFAnomalyDetector(BaseAnomalyDetector):
    ...
    def _build_model(self):
        ...
        try:
            ...
            # Using a simpler implementation with fewer weights to reduce compatibility issues
            initializer = tf.random.normal
            self.weights = {
                'encoder': tf.Variable(initializer([self.input_dim, self.latent_dim])),
                'encoder_bias': tf.Variable(tf.zeros([self.latent_dim])),
                'decoder': tf.Variable(initializer([self.latent_dim, self.input_dim])),
                'decoder_bias': tf.Variable(tf.zeros([self.input_dim])),
            }
        except Exception as e:
    ...
    def _autoencoder(self, x):
        ...
        try:
            # Simple encoder
            encoded = tf.nn.sigmoid(tf.matmul(x, self.weights['encoder']) + self.weights['encoder_bias'])
            # Simple decoder
            decoded = tf.matmul(encoded, self.weights['decoder']) + self.weights['decoder_bias']
            return decoded
        except Exception as e:
            print(f"Error in TensorFlow autoencoder: {str(e)}")
            return x
    ...
    def fit(self, X):
        ...
        try:
            ...
            # Use tensorflow's GradientTape for training
            optimizer = tf.optimizers.Adam(learning_rate=self.learning_rate)
            
            # Simple training loop
            for epoch in range(self.epochs):
                # Use a simple approach without batching for fewer compatibility issues
                with tf.GradientTape() as tape:
                    # Forward pass
                    reconstructed = self._autoencoder(X_tensor)
                    # Compute loss - mean squared error
                    loss = tf.reduce_mean(tf.square(reconstructed - X_tensor))
    ...
    def predict(self, X):
        if not TF_AVAILABLE or not self.weights or not self.trained:
            print("TensorFlow is not available or model failed to train. Cannot predict.")
            return np.zeros(X.shape[0])
