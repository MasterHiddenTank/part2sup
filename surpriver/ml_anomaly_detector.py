...
try:
    ...
    # Try to avoid OpenSSL import altogether if possible
    os.environ["TF_USE_LEGACY_TENSORFLOW"] = "1"
    
    # Attempt to import TensorFlow with a timeout for safety
    import tensorflow as tf
    
    # Basic check if TensorFlow is actually functional
    # Create a simple tensor to verify TensorFlow core functionality works
...
class TensorFlowAnomalyDetector(BaseAnomalyDetector):
    ...
    def _build_model(self):
        """Define autoencoder architecture"""
        # Guard against potential TensorFlow errors
        if not TF_AVAILABLE:
            return
        ...
        try:
            ...
            print(f"Failed to build TensorFlow model: {str(e)}")
            self.model = None
...
class MLAnomalyDetectionSystem:
    ...
    def create_models(self, input_dim):
        ...
        if TF_AVAILABLE:
            models["TFAutoencoder"] = TensorFlowAnomalyDetector(input_dim=input_dim)
