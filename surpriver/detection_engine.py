...
# Try to import the ML anomaly detector with graceful fallback
ML_SYSTEM_AVAILABLE = False
try:
    from ml_anomaly_detector import MLAnomalyDetectionSystem
    ML_SYSTEM_AVAILABLE = True
except Exception as e:
    print(f"Warning: ML Anomaly Detection System could not be loaded: {str(e)}")
    print("Falling back to Isolation Forest only.")
    ML_SYSTEM_AVAILABLE = False
...
argParser.add_argument("--ml_model", type=str, default = "isolation_forest", help="ML model to use for anomaly detection: isolation_forest, pytorch, tensorflow, xgboost, ensemble, best")
...
ml_model = args.ml_model.lower()

"""
Sample run:
python detection_engine.py --is_test 1 --future_bars 25 --top_n 25 --min_volume 5000 --data_granularity_minutes 60 --history_to_use 14 --is_load_from_dictionary 0 --data_dictionary_path 'dictionaries/feature_dict.npy' --is_save_dictionary 1 --output_format 'CLI' --ml_model 'ensemble'
"""
...
class ArgChecker:
	...
	def check_arugments(self):
		...
		if not path.exists(directory_path + f'/stocks/{stock_list}'):
			...
			exit()
		if data_source not in ['binance', 'yahoo_finance', 'mock']:
			print("Data source must be a valid and supported service.")
			exit()
		
		# Check ML model type
		valid_ml_models = ['isolation_forest', 'pytorch', 'tensorflow', 'xgboost', 'ensemble', 'best']
		if ml_model not in valid_ml_models:
			print(f"ML model must be one of: {', '.join(valid_ml_models)}. Using 'isolation_forest' as default.")
			ml_model = 'isolation_forest'
			
		# Force isolation_forest if ML system is not available
		if not ML_SYSTEM_AVAILABLE and ml_model != 'isolation_forest':
			print("ML Anomaly Detection System is not available. Falling back to isolation_forest.")
			ml_model = 'isolation_forest'
...
class Surpriver:
	...
	def find_anomalies(self):
		...
		if self.IS_LOAD_FROM_DICTIONARY == 0:
			print(f"Collecting data from {self.DATA_SOURCE}...")
			features, historical_price_info, future_prices, symbol_names = self.dataEngine.collect_data_for_all_tickers()
		else:
			# Load data from dictionary
			print(f"Loading data from dictionary: {self.DATA_DICTIONARY_PATH}")
			features, historical_price_info, future_prices, symbol_names = self.dataEngine.load_data_from_dictionary()
		
		print(f"Successfully loaded data for {len(symbol_names)} symbols")
		
		# Choose the anomaly detection model based on user input
		if self.ML_MODEL == 'isolation_forest' or not ML_SYSTEM_AVAILABLE:
			...
			# Use the advanced ML-based anomaly detection system
			try:
				print(f"Using ML model: {self.ML_MODEL}")
				ml_detector = MLAnomalyDetectionSystem(method=self.ML_MODEL)
				ml_detector.fit(np.array(features))
				print("Generating anomaly scores...")
				predictions = ml_detector.predict(np.array(features))
			except Exception as e:
				print(f"Error using ML model: {str(e)}")
				print("Falling back to Isolation Forest model")
				detector = IsolationForest(n_estimators=100, random_state=0)
				detector.fit(features)
				predictions = detector.decision_function(features)
		
		print(f"Sorting {len(predictions)} predictions...")
		...
		print(f"Preparing results for top {self.TOP_PREDICTIONS_TO_PRINT} predictions...")
			...
			if self.IS_TEST == 0:
				...
				if self.OUTPUT_FORMAT == "CLI":
					print("Last Bar Time: %s\nSymbol: %s\nAnomaly Score: %.3f\nToday Volume: %s\nAverage Volume 5d: %s\nAverage Volume 20d: %s\nVolatility 5bars: %.3f\nVolatility 20bars: %.3f\nFuture Absolute Sum Price Changes: %.2f\nML Model: %s\n----------------------" % 
																	...
																	today_volume, average_vol_last_five_days, average_vol_last_twenty_days,
																	volatility_vol_last_five_days, volatility_vol_last_twenty_days,
																	future_abs_sum_percentage_change, self.ML_MODEL))
				results.append({
					'latest_date' : latest_date,
					'Symbol' : symbol,
					'Anomaly Score' : prediction,
	...
	def store_results(self, results):
		...
		file_name = '%s_%s_%s.json' % (prefix, self.ML_MODEL, str(today))
	...
	def calculate_future_stats(self, predictions_with_output_data):
		...
		print("\n*************** Future Performance ***************")
		print("ML Model: %s" % self.ML_MODEL)
		...
		plt.title(f"Absolute Future Change - {self.ML_MODEL}", fontsize = FONT_SIZE)
		plt.tight_layout()
		plt.grid()
		plt.show()
